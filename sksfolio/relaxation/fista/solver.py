...
def _settings(options: Optional[Any]) -> Tuple[_Settings, int]:
    ...
    backtracking_factor = float(supplied.get("backtracking_factor", 2.0))
    ...
    prox_target_ceiling = float(supplied.get("prox_target_ceiling", 1e-3))
    prox_max_iterations = int(
...
def solve_fista(
    ...
    result["threadpool_controller"] = (
        "per_solve_threadpoolctl_discovery" if threads else "not_used"
    )
