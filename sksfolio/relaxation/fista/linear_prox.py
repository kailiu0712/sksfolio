...
class LinearConstraintProx:
    ...
    def __init__(
        ...
        *,
        ...
        self._scaled_finite_lower = np.flatnonzero(
            np.isfinite(self.scaled_lower)
        )
        self._scaled_finite_upper = np.flatnonzero(
            np.isfinite(self.scaled_upper)
        )
        self._original_finite_lower = np.flatnonzero(
            np.isfinite(self.lower)
        )
        self._original_finite_upper = np.flatnonzero(
        ...
        self._lbfgs_equal_values = self.scaled_lower[
            self._lbfgs_equal
        ].copy()
        self._lbfgs_upper_values = self.scaled_upper[
            self._lbfgs_upper
        ].copy()
        self._lbfgs_lower_values = self.scaled_lower[
            self._lbfgs_lower
        ].copy()
    ...
    @staticmethod
    def _cached_interval_violation(
        value: np.ndarray,
        lower: np.ndarray,
        upper: np.ndarray,
        ...
        """Return interval violation without rebuilding finite-bound masks."""
        violation = 0.0
        for index in finite_lower:
            violation = max(
                violation,
                float(lower[index] - value[index]),
            )
        for index in finite_upper:
            violation = max(
                violation,
                float(value[index] - upper[index]),
            )
        return max(violation, 0.0)
    ...
    def _lbfgs_dual(self, coordinates: np.ndarray) -> np.ndarray:
        ...
        dual = np.zeros(self.rows, dtype=float)
        return self._lbfgs_dual_into(coordinates, dual)

    def _lbfgs_dual_into(
        self,
        coordinates: np.ndarray,
        dual: np.ndarray,
    ) -> np.ndarray:
        """Map split L-BFGS-B coordinates into a reusable dual buffer."""
        dual.fill(0.0)
        cursor = 0
