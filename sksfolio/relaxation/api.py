"""Public API for corrected and optional commercial relaxation backends."""
...
BACKENDS = CORRECTED_ALGORITHMS + ("hybrid_newton",) + COMMERCIAL_BACKENDS
...
def registered_backends() -> tuple[str, ...]:
    """Return corrected and optional commercial backend identifiers."""
    return BACKENDS


def available_backends() -> tuple[str, ...]:
    """Return backend identifiers understood by the public API."""
    return BACKENDS
...
def _backend_solver(backend: str) -> Any:
    resolved = _COMMERCIAL_ALIASES.get(backend, backend)
    family, language = resolved.split(".", 1)
    module = importlib.import_module(
        f"sksfolio.relaxation.{family}.{language}"
    )
    return getattr(module, "solve"), resolved
...
def solve_relaxation(
    ...
    *,
    ...
    """Solve with corrected first-order or optional commercial backends."""
    ...
    if algorithm in _PROX_ORACLE:
        ...
        if algorithm == "hybrid_newton":
            supplied["prox_newton"] = True
        if (
        ...
        resolved_backend = algorithm
        language = "python"
        implementation = "python"
        native_solver_core = False
    else:
        if warm_start is not None:
            ...
            supplied["warm_start"] = (
                ...
                else RelaxationState.coerce(warm_start)
            )
        solver, resolved_backend = _backend_solver(algorithm)
        raw = dict(solver(instance, supplied))
        family, _, language = resolved_backend.partition(".")
        implementation = language or "python"
        native_solver_core = False
    ...
    raw.setdefault("canonical_backend", resolved_backend)
    raw.setdefault("algorithm", algorithm)
    raw.setdefault("language", language)
    raw.setdefault("implementation", implementation)
    raw.setdefault("native_solver_core", native_solver_core)
    ...
    if raw.get("x") is not None:
        ...
        diagnostics = evaluate_solution(
            instance,
            weights,
            domain_tolerance=diagnostic_tolerance,
        )
        raw["diagnostics"] = diagnostics
        raw.setdefault("evaluation", diagnostics)
        raw["external_objective"] = diagnostics["objective"]
        _attach_safe_dual_certificate(instance, weights, raw)
        maximum_violation = float(
            diagnostics["violations"]["maximum"]
        )
